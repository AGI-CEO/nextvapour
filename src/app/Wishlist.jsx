...
const Wishlist = () => {
  ...
  const playCoinSound = () => {
    const audio = new Audio("/assets/MarioCoinSound.mp3");
    audio.volume = 0.4;
    audio.play();
  };
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/games/${searchQuery}`);
      const gameData = await response.json();
      console.log(gameData[0]);
      if (gameData.length > 0) {
        const firstGame = gameData[0];

        const newWishlistItem = await fetch("/api/videogames", {
          method: "POST",
          headers: {
        ...
        const newWishlistItemData = await newWishlistItem.json();
        setWishlist([...wishlist, newWishlistItemData]);
        setSearchQuery("");
      } else {
  ...
  const getAndSetGameSuggestions = async (inputValue) => {
    try {
      if (inputValue) {
        const response = await fetch(`/api/games/${inputValue}`);
        const gameData = await response.json();
  ...
  const deleteGame = async (gameId) => {
    try {
      await fetch(`/api/videogames/${gameId}`, {
        method: "DELETE",
      });
